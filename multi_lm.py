...
from model import MultiLingualMultiDomainLM, Discriminator
...
def main():
    ...
    parser.add_argument('--val_metric', nargs='+', default=['en-fr', 'en-de', 'en-ja'], help='validation metric')
    ...
    for metric in args.val_metric:
        sl, tl = metric.split('-')
        sid, tid = lang2id[sl], lang2id[tl]
        sv, tv = vocabs[lang2id[sl]], vocabs[lang2id[tl]]
        lex, lexsize = load_lexicon('data/muse/{}.0-5000.txt'.format(metric), sv, tv)
        lexicons.append((lex, lexsize, sid, tid))
    ...
    if args.resume:
        ...
        model = MultiLingualMultiDomainLM(n_langs=len(args.lang), n_doms=len(args.dom), n_tok=list(map(len, vocabs)),
    ...
    try:
        ...
        for epoch in range(args.epochs):
            ...
            if (epoch + 1) % args.log_interval == 0:
                ...
                print('| epoch {:4d} | lm_lr {:05.5f} | ms/batch {:6.2f} | loss {:5.2f} | avg_ppl {:7.2f} | dis {:7.4f} |'.format(
                    epoch, lm_opt.param_groups[0]['lr'], elapsed * 1000 / args.log_interval,
                    total_loss.mean(), np.exp(total_loss).mean(), np.nan))
            ...
            if (epoch + 1) % args.val_interval == 0:
                model.eval()
                accs = []
                for lex, lexsz, si, ti in lexicons:
                    sw = model.encoder_weight(si).cpu().numpy()
                    tw = model.encoder_weight(ti).cpu().numpy()
                    acc = compute_nn_accuracy(sw, tw, lex, lexicon_size=lexsz)
                    accs.append(acc)

                print('-' * LINE_WIDTH)
                print(('| epoch {:4d} | loss {:5.2f} | dis {:5.4f} |' + ' {} {:6.4f} |' * len(accs)).format(
                    epoch, np.nan, np.nan, *sum([[metric, acc] for metric, acc in zip(args.val_metric, accs)], [])))
                print('-' * LINE_WIDTH)
