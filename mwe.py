...
def model_save(model, lm_opt, path):
    with open(path, 'wb') as f:
        torch.save([model, lm_opt], f)


def model_load(path):
    with open(path, 'rb') as f:
        model, lm_opt = torch.load(f)
    return model, lm_opt
...
def main():
    ...
    # load multi-lingual word embeddings
    mwe = []
    oov_rates = []
    for v, lang in zip(vocabs, args.lang):
        x, count = load_vectors_with_vocab(args.embedding.format(lang), v, -1)
        mwe.append(x)
    ...
    for lang, oov_rate in zip(args.lang, oov_rates):
    ...
    if args.resume:
        model, lm_opt = model_load(args.resume)
        ...
        for lid in range(len(args.lang)):
            ...
            if (epoch + 1) % args.val_interval == 0:
                ...
                for tlang, val_acc in zip(args.trg, val_accs):
                    if val_acc > best_accs[tlang]:
                        ...
                        model_save(model, lm_opt, save_path)
    ...
    for tid, tlang, ds in zip(trg_ids, args.trg, test_ds):
        save_path = model_path.replace('.pt', '_{}.pt'.format(tlang))
        model, lm_opt = model_load(save_path)   # Load the best saved model.
        model.eval()
