...
def evaluate(data_source, batch_size, model, criterion):
...
def train(train_data, model, optimizer, args, criterion, params, epoch):
...
def main():
    ...
    val_data = batchify(valid_set, eval_batch_size)
    ...
    try:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            train(train_data, model, optimizer, args, criterion, params, epoch)
            ...
            if 't0' in optimizer.param_groups[0]:
                ...
                val_loss2 = evaluate(val_data, 10, model, criterion)
                print('-' * 89)
                ...
                val_loss = evaluate(val_data, eval_batch_size, model, criterion)
                print('-' * 89)
                print('| end of epoch {:3d} | time: {:5.2f}s | valid loss {:5.2f} | '
                      'valid ppl {:8.2f} | valid bpc {:8.3f}'.format(
    ...
    # Run on test data.
    test_loss = evaluate(test_data, test_batch_size, model, criterion)
    print('=' * 89)
